...
class CardExpanded extends React.Component {
  ...
  render() {
    ...
    const cardTitle = <li id='-1'><h2>{this.props.results.name ? this.props.results.name : this.props.results.title}</h2></li>;
  ...
  return (
    ...
    <ul className="card-expanded" onClick={this.props.expandCard}>
      ...
      {selectedCategory.map((value, index) => {
        ...
        console.log('this.props.results[value.name]' , this.props.results[value.name])
        if (value.name === 'description') {
          return <li id={index}>{this.props.results[value.name]}</li>
        
        } else if (value.unit) { // If the value has a unit (set as state variable)
          if (value.name === 'age' && isNaN(this.props.results[value.name])) { // Special Case - do not use the unit (years) when age is empty or N/A
            return <li id={index}><h7>{value.label}:</h7> {this.props.results[value.name]}</li>
            ...
            return <li id={index}><h7>{value.label}:</h7> {this.props.results[value.name]} {rottenTomatoes}</li>
            ...
            if (this.props.results[value.name] > 0) {
              return <li id={index}><h7>{value.label}:</h7> {this.props.results[value.name]} {value.unit}<sup>2</sup></li>
            ...
            return <li id={index}><h7>{value.label}:</h7> {this.props.results[value.name]} {value.unit}</li>
          }
